...
class Test
{
  private:
    ...
    constexpr static const size_t MAX_ARR_SIZE = 35;
    ...
    static void print_check(It begin, It end, int max_w, int w = DEFAULT_W);
    template <typename C>
  ...
  public:
    ...
    template <typename F>
    static void test_nocomp(F sort, int examples = 10);
...
template <typename It>
template <typename F>
void Test<It>::test_nocomp(F sort, int examples)
{
  std::srand(std::time(NULL));
  ...
  for (int i = 0; i < examples; ++i)
  {
    ...
    std::cout << "Sorted:   ";
    Test<>::print_check(vec.begin(), vec.end(), max_w);
    if (i != examples - 1)
      std::cout << std::endl;
  }
}
...
template <typename It>
void Test<It>::print_check(It begin, It end, int max_w, int w)
{
  std::ostringstream oss;
  for (It it = begin; it != end; ++it)
    oss << std::setw(w) << std::right << *it;

  std::cout << std::setw(max_w) << std::left << oss.str() << (Test<>::check_sorted(begin, end) ? "(T)" : "(F)") << std::endl;
